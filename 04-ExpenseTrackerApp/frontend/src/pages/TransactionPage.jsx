/*
- This code defines a React component for updating a transaction.
- It fetches transaction details using a GraphQL query.
- The component handles form inputs and updates the transaction using a mutation.
- It includes error handling and displays success/error messages.
- The form includes fields for description, payment type, category, amount, location, and date.
*/
...
const TransactionPage = () => {
	...
	// Fetching the transaction data using Apollo's useQuery hook
	const { loading, data } = useQuery(GET_TRANSACTION, {
		variables: { id: id },
	});

	// Logging the fetched transaction data
	console.log("Transaction", data);

	// Setting up the mutation for updating the transaction
	const [updateTransaction, { loading: loadingUpdate }] = useMutation(UPDATE_TRANSACTION, {
		// Refetching transaction statistics after updating the transaction
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		const amount = parseFloat(formData.amount); // Converting amount to a number

		try {
			// Executing the update transaction mutation
	...
	// Handling input changes and updating form data state
	...
	// Updating form data state when transaction data is fetched
	useEffect(() => {
		if (data) {
			setFormData({
				description: data?.transaction?.description,
	...
	// Displaying a skeleton loader while the data is loading
